...
var App = function () {
    ...
    var handleSessionAlive = function () {
    	$.ajax({
    		url : '/ajax.do?request=checkSession',
    		type : 'POST',
    ...
    var handleSidebarMenu = function () {
        ...
        jQuery('.page-sidebar').on('click', ' li > a.ajaxify', function (e) {
            ...
            handleSessionAlive();//check session alive
            historyPage=url;
    ...
    var handleSidebarToggler = function () {
		...
		*/
        ...
        $('.sidebar-search .submit').on('click', function (e) {
        	var transactionId = $.trim($('#searchId').val());
            ...
            if(transactionId.length != 12){
        		bootbox.alert('Transaction Id is not valid');
        		return;
        	}else{
        		trnsctnDetail('#search-modal',transactionId,'700px');
        	}
        });
    ...
    // Handle PGMATE ELEMENT
    var handlePgmate = function() {
    	$('.date-picker').datepicker({
    		rtl: App.isRTL(),
            autoclose: true
        });
    	/*
    	//Ctrl+B 에 대한 이벤트 제어 아직 못함.
		...
		*/
    	...
    	$('#searchForm').live('keypress', function(e) {
    	...
    	$('input.merchantId').typeahead({
    		source: function (query, process) {
    			$.ajax({
    				url: '/ajax.do',
    				type: 'POST',
    				dataType: 'JSON',
    				data: 'request=merchantId&merchantId=' + query,
    	...
    	$(document).on("keyup", "input:text[numberHypen]", function() {$(this).val( $(this).val().replace(/[^0-9\-]/gi,"") );});
