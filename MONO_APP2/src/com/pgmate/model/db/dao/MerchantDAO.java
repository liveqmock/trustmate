...
public class MerchantDAO extends DAO {
	...
	public MerchantBean getByGroupCheckMerchantId(String groupId, String merchantId){
		List<MerchantBean> list = get(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID='"+groupId+"') AND MERCHANT_ID ='"+merchantId+"' ");
		if(list.size() == 0){
			return new MerchantBean();
		}else{
			return (MerchantBean)list.get(0);
	...
	public List<MerchantBean> get(MerchantBean merchantBean){
		...
		if(merchantBean.getIdx() != 0){
			...
			if(!merchantBean.getPublicAgentId().equals("")){
				sb.append(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_AGENT_MERCHANT WHERE AGENT_ID='"+merchantBean.getPublicAgentId()+"')");
			}
			if(!merchantBean.getPublicGroupId().equals("")){
				sb.append(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID='"+merchantBean.getPublicGroupId()+"')");
			}
	...
	public List<String> getMerchantList(String merchantId){
		
		String query = "SELECT MERCHANT_ID FROM VW_MERCHANT WHERE MERCHANT_ID LIKE '%"+merchantId+"%' ORDER BY MERCHANT_ID DESC";
