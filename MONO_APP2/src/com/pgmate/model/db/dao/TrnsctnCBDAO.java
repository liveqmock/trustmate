...
public class TrnsctnCBDAO extends DAO {
	...
	public List<TrnsctnCBBean> getView(TrnsctnCBBean trnsctncbBean){
		...
		if(!trnsctncbBean.getTemp2String().equals("")){
			sb.append(" AND B.VAN_TRANSACTION_ID = '"+ trnsctncbBean.getTemp2String() +"'");
		}
		...
		if(!trnsctncbBean.getTemp3String().equals("")){
			sb.append(" AND B.MERCHANT_ID = '"+ trnsctncbBean.getTemp3String() +"'");
		}
		
		
		if(!trnsctncbBean.getPublicAgentId().equals("")){
			sb.append(" AND B.MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_AGENT_MERCHANT WHERE AGENT_ID='"+trnsctncbBean.getPublicAgentId()+"')");
		}
		if(!trnsctncbBean.getPublicGroupId().equals("")){
			sb.append(" AND B.MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID='"+trnsctncbBean.getPublicGroupId()+"')");
		}
