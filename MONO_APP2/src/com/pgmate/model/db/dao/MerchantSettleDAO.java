...
public class MerchantSettleDAO extends DAO {
	...
	public List<MerchantSettleBean> get(MerchantSettleBean merchantSettleBean){
		...
		if(!merchantSettleBean.getSettleCnt().equals("")){
			sb.append(" AND SETTLE_CNT = '"+merchantSettleBean.getSettleCnt()+"' ");
		}
		...
		if(!merchantSettleBean.getPublicAgentId().equals("")){
			sb.append(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_AGENT_MERCHANT WHERE AGENT_ID='"+merchantSettleBean.getPublicAgentId()+"')");
		}
		if(!merchantSettleBean.getPublicGroupId().equals("")){
			sb.append(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID='"+merchantSettleBean.getPublicGroupId()+"')");
		}
