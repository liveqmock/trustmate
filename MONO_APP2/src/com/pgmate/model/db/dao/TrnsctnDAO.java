...
public class TrnsctnDAO extends DAO {
	...
	public List<TrnsctnBean> getRecentError(){
		return getRecentError("");
	}
	
	public List<TrnsctnBean> getRecentError(String where){
		List<TrnsctnBean> list = new ArrayList<TrnsctnBean>();
		String query = "SELECT * FROM ( "
					+" SELECT ROW_NUMBER() OVER(ORDER BY IDX DESC) AS RNUM, TRANSACTION_ID,MERCHANT_ID,AMOUNT,RESULT_MSG,FC_CODE('KAPPV_CODE',RESULT_MSG,'en') AS MSG,TRN_REQ_DATE "
					+" FROM TB_TRNSCTN WHERE RESULT_CD='1' "+where + " AND TRN_DATE >= TO_CHAR(SYSDATE-5,'YYYYMMDD') )  WHERE RNUM>= 1 AND RNUM <= 5";
		...
		try{
			...
			while(rset.next()){
				...
				trnsctnBean.setAmount(rset.getDouble("AMOUNT"));
				trnsctnBean.setResultMsg(CommonUtil.nToB(rset.getString("RESULT_MSG")));
				trnsctnBean.setTemp1String(CommonUtil.nToB(rset.getString("MSG")));
				trnsctnBean.setTrnReqDate(rset.getTimestamp("TRN_REQ_DATE"));
				list.add(trnsctnBean);
			}
		}catch(Exception e){
	...
	public List<TrnsctnBean> getRecentTransaction(String where){
		List<TrnsctnBean> list = new ArrayList<TrnsctnBean>();
		String query = "SELECT * FROM ( "
					+" SELECT ROW_NUMBER() OVER(ORDER BY IDX DESC) AS RNUM, TRANSACTION_ID,MERCHANT_ID,AMOUNT,RESULT_MSG,FC_CODE('KAPPV_CODE',RESULT_MSG,'en') AS MSG,TRN_REQ_DATE "
					+" FROM TB_TRNSCTN WHERE 1=1 "+where + " AND TRN_DATE >= TO_CHAR(SYSDATE,'YYYYMMDD') )  WHERE RNUM>= 1 AND RNUM <= 5";
