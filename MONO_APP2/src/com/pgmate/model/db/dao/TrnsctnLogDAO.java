...
public class TrnsctnLogDAO extends DAO {
	...
	public List<TrnsctnLogBean> getTrnsctnLog(TrnsctnLogBean trnsctnLogBean){
		...
		if(trnsctnLogBean.getIdx() !=0){
			...
			if(!trnsctnLogBean.getPublicAgentId().equals("")){
				sb.append(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_AGENT_MERCHANT WHERE AGENT_ID='"+trnsctnLogBean.getPublicAgentId()+"')");
			}
			if(!trnsctnLogBean.getPublicGroupId().equals("")){
				sb.append(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID='"+trnsctnLogBean.getPublicGroupId()+"')");
			}
