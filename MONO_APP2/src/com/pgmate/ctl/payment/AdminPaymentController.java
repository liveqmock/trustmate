	...
	private SSOBean ssoBean = null;
		...
		public ModelAndView handleRequest(HttpServletRequest request,HttpServletResponse response) throws Exception {
			...
			try{
				...
				}else if(param.getString("REQ_TYPE").equals("refund")){
					mav = refund(param);
				}else if(param.getString("REQ_TYPE").equals("cbAdd")){
					mav = cbAdd(param);
				}else{
		...
		public ModelAndView creditPayment(ParamUtil param){
			...
			param.setAttribute("resultMsg",new CodeDAO().getByAliasCode("KAPPV_CODE",headerBean.getResultMsg()).getEnValue());
		...
		public ModelAndView refund(ParamUtil param){
			TrnsctnDAO trnsctnDAO = new TrnsctnDAO();
			TrnsctnBean trnsctnBean = trnsctnDAO.getByTransactionId(param.getString("transactionId"));
			...
			Log.debug("log.day","거래 취소 요청 거래번호 =["+tBean.getRTransactionId()+"] 사용자=["+ssoBean.getMemberId()+"]",this);
			if(headerBean.getResultMsg().equals("0000")){
				Log.debug("log.day","신용카드 거래취소  성공",this);
				TrnsctnVoidBean trnsctnvoidBean = new TrnsctnVoidDAO().getByTransactionId(tBean.getRTransactionId());
				new Comment().updateTrnsctnComment(trnsctnvoidBean.getCommentIdx(),ssoBean.getMemberId(),param.getString("comment")+" ("+param.getRemoteAddr()+")");
				return new ModelAndView("/common/ajaxResponse","message","OK||[Credit card transaction cancellation acceptance completion] TRANSACTIONID =["+trnsctnvoidBean.getTransactionId() +"]");
			}else{
				Log.debug("log.day","신용카드 거래취소 실패",this);
				String msg = new CodeDAO().getByAliasCode("KAPPV_CODE",trnsctnBean.getResultMsg()).getEnValue();
				return new ModelAndView("/common/ajaxResponse","message","NOK||[Credit card transaction cancellation acceptance failure] "+msg);
			}
			
		}
		
		
		public ModelAndView cbAdd(ParamUtil param){
			TrnsctnDAO trnsctnDAO = new TrnsctnDAO();
			TrnsctnBean trnsctnBean = trnsctnDAO.getByTransactionId(param.getString("transactionId"));
			
			TrnsctnCBBean tcBean = new TrnsctnCBBean();
			tcBean.setTransactionId(trnsctnBean.getTransactionId());
			tcBean.setFlag("N");
			tcBean.setCbState("3");
			tcBean.setRootTrnStatus(trnsctnBean.getTrnStatus());
			
			if(new TrnsctnCBDAO().insert(tcBean)){
				if(new TrnsctnDAO().updateTrnStatus(tcBean.getTransactionId(), "23")){
					new Comment().setTrnsctnComment(tcBean.getCommentIdx(),ssoBean.getMemberId(),param.getString("comment")+" ("+param.getRemoteAddr()+")");
					return new ModelAndView("/common/ajaxResponse","message","OK||[ChargeBack has been registed] TRANSACTIONID =["+trnsctnBean.getTransactionId() +"]");
				}else{
					param.setAttribute("redirectURL","/report.do?request=cblist");
					return new ModelAndView("/common/ajaxResponse","message","NOK||ChargeBack failed to register. ");
				}
			}else{
				param.setAttribute("redirectURL","/report.do?request=cblist");
				return new ModelAndView("/common/ajaxResponse","message","NOK||ChargeBack failed to register.");
		...
		public byte[] connect(byte[] request){
			...
			socket.setSocketInfo("112.175.48.41","20000");
