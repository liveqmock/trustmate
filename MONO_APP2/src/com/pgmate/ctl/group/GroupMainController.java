	...
	public ModelAndView handleRequest(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try{
			if(param.getSession("sso") == null){
				...
				param.setAttribute("recentErrorTrnsctn",new TrnsctnDAO().getRecentError(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID ='"+ssoBean.getMemberId()+"') "));
				param.setAttribute("recentTrnsctn",new TrnsctnDAO().getRecentTransaction(" AND MERCHANT_ID IN (SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID ='"+ssoBean.getMemberId()+"') "));
