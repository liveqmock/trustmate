...
public class TrnsctnGroupDAO extends DAO {
	
	public double sumAmount = 0;
	public String searchQuery = "";
	
	public TrnsctnGroupDAO(){
	...
	public List<TrnsctnBean> get(TrnsctnBean trnsctnBean){
		...
		if(!trnsctnBean.getTemp3String().equals("")){
            sb.append(" AND a.MERCHANT_ID IN ( SELECT MERCHANT_ID FROM TB_GROUP_MERCHANT WHERE GROUP_ID ='"+ trnsctnBean.getTemp3String() +"') ");
		}
	...
	public CurrentTrnsctnBean getMonthlyTransaction(String curType){
		...
		String query = "SELECT COUNT(*) AS CNT, SUM(AMOUNT) AS SUM FROM VW_TRNSCTN WHERE TRN_STATUS IN ('02','07','08','09','11','12','13','10')"
					+"	AND TO_CHAR(TRN_REQ_DATE,'YYYYMM') = TO_CHAR(SYSDATE,'YYYYMM') AND CUR_TYPE = '"+curType+"' ";
		DBFactory db 	= null;
		PreparedStatement pstmt	= null;
		Connection conn			= null;
		ResultSet rset			= null;
