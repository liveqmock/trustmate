 ...
 * Project Name : PG_APP
 * Project      : PG_APP
 * File Name    : com.pgmate.model.db.dao.CommentServiceDAO.java
 * Date	        : Dec 23, 2008
 * Version      : 1.0
...
public class CommentServiceDAO extends DAO {

	public CommentServiceDAO() {
	}
	
	
	public List<CommentServiceBean> getByMemberId(String memberId){
		return get(" AND MEMBER_ID = '"+memberId+"'");
	}
	
	public List<CommentServiceBean> getByIdx(String idx){
		return get(" AND IDX ="+idx);	
	}
	
	public List<CommentServiceBean> get(String subQuery){
		String query = "SELECT IDX, MEMBER_ID, COMMENTS, REG_DATE FROM VW_COMMENT_SERVICE WHERE IDX IS NOT NULL " + subQuery;
		...
		List<CommentServiceBean> list = new ArrayList<CommentServiceBean>();
		DBFactory db 	= null;
		...
		try{
			...
			while(rset.next()){
				CommentServiceBean csBean = new CommentServiceBean();
	...
	public boolean update(CommentServiceBean csBean){
		String query = "UPDATE TB_COMMENT_SERVICE SET MEMBER_ID = ?,COMMENTS  =?, WHERE IDX=?";
	...
	public boolean insert(CommentServiceBean csBean){
		String query = "INSERT INTO TB_COMMENT_SERVICE (IDX, MEMBER_ID, COMMENTS,REG_DATE) VALUES (? ,? ,? ,SYSDATE )";
